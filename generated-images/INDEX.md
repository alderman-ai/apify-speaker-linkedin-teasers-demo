...
One finished 1200×1200 PNG per speaker, named `<speaker-name>.png`.
...
`generated-images/<speaker-name>.png` (the render itself).

**Nothing here is ever overwritten** — re-rendering a speaker requires
moving or renaming their existing final first.

`alex-alderman.png` is the author's own card, the live worked example; a
second card for the same name lands beside it as `alex-alderman-<NN>.png`.
