...
speaker_name:       "Baba Yaga"
speaker_position:   "hut-architect"
speaker_company:    "chicken-leg-systems"
...
# frontmatter, and §2.
topic_category:     "Self-Relocating Infrastructure"

# --- 5. talk metadata (card footer, right) -- WRITTEN BY ASSISTANT -----------
level:              "adv."              # all | easy | int. | adv.
duration_minutes:   "25"              # number only — the card renders 10 (mins)
...
```topic-category
Self-Relocating Infrastructure
```
...
My hut has legs, so it never waits on a data centre. A field guide to infrastructure that stands up, turns round and walks away.
```
