...
processed/<speaker>/     the whole folder moves here on success, PNG inside
```
...
121.667 · 1 line 137.667 · 2 lines 153.667 · 3 lines 169.667. (A ≤82-char
description at width 400 renders 2 lines when over ~45 chars, 1 line under.)
...
### 5 · Marked photos → focus

If `speaker_image` is not already the block's size and the operator drew a
visible mark on it (an "x", a scribble, any annotation): find the mark's
pixel position and set frontmatter `speaker_focus: [x, y]`. Convert to the
shell's `{{SPK_POS}}` as `X% Y%` where `X = x/imageW×100`. No mark → `50%
50%`. The original image is never modified — the crop is pure CSS
